...
//APITracer::Start(instance, APITracer::OutputFile);
		...
		if (!IsUnload) hook_initinternal();	//ｲｻｼﾓﾔﾘｵﾄﾄ｣ｿ鮴ﾍｲｻﾗﾎｺﾎﾊﾂﾇ・
		//5
...
//			InstallManagerHook();
		...
		if (IsUnload)
		{
			...
			BOOL HookMode = (mutex_offical || (mutex_gditray2 && mutex_CompMode)) || (!mutex_offical && !mutex_gditray2);	//ﾊﾇｷﾚｼ貶ﾝﾄ｣ﾊｽﾏﾂ
			CloseHandle(mutex_CompMode);
			CloseHandle(mutex_gditray2);
			CloseHandle(mutex_offical);
			if (!HookMode)	//ｷﾇｼ貶ﾝﾄ｣ﾊｽﾏﾂ｣ｬｾﾜｾﾓﾔﾘ
				return false;
		}
...
//		RemoveManagerHook();
		if (!bDllInited)
			return true;
		bDllInited = false;
		if (InterlockedExchange(&g_bHookEnabled, FALSE) && lpReserved == NULL) {	//ﾈ郢鈹ﾇｽﾌﾖﾕﾖｹ｣ｬﾔｻﾐ靨ｪﾊﾍｷﾅ
			hook_term();
			//delete AACacheFull;
			//delete AACache;
// 			for (int i=0;i<CACHE_SIZE;i++)
// 				delete g_AACache2[i];	//貂�髯､郛鍋｣･E
			//free(g_charmapCache);
		}
#ifndef DEBUG
