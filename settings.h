...
#include "hooklist.h"
#undef HOOK_DEFINE	//‰∏∫‰∫ÜÁ°Æ‰øùÊ≠§Êñá‰ª∂‰πüËÉΩÁõ¥Êé•‰ΩøÁî®Ê≠£Á°ÆÁöÑÂáΩÊï∞ÔºåÈúÄË¶ÅÁî≥Êòé„ÄÇ
#undef HOOK_MANUALLY
...
public:
	enum {
		INFOMAX = 180,	//Ê∫êÊñá‰ª∂=15
		FONTMAX = 31,
	};
private:
	LPWSTR info[INFOMAX + 1][FONTMAX + 1];
	bool AllowDefaultLink[256];
	WCHAR DefaultFontLink[FF_DECORATIVE + 1][LF_FACESIZE + 1];	//¥Ê∑≈∂‘”¶◊÷ÃÂ¿‡–Õµƒƒ¨»œ¡¥Ω”
public:
	CFontLinkInfo();
	~CFontLinkInfo();
	void init();
...
//	bool m_bHaveNewerFreeType		: 1;
	bool							: 0;
	bool m_bUseCustomLcdFilter;	// π”√◊‘∂®“Âlcdfilter
	...
	int	 m_bFontLink;	//Êîπ‰∏∫ÂèØ‰ª•‰ΩøÁî®Â§öÁßçÂèÇÊï∞
	int  m_nWidthMode;
	int  m_nFontLoader;
	...
	// „Ç¢„É≥„ÉÅ„Ç®„Ç§„É™„Ç¢„ÇπË™øÊï¥Áî®„ÉÜ„Éº„Éñ„É´
...
//	bool EnableKerning() const { return m_bEnableKerning; }
	...
	int FontSubstitutes() const { return m_nFontSubstitutes; }	//≈–∂œÃÊªªƒ£ Ω
...
//	bool HaveNewerFreeType() const { return m_bHaveNewerFreeType; }
	...
	void AddFontExclude(LPCWSTR lpFaceName);	//ÁÇπÈòµÂ≠ó‰ΩìÁõ¥Êé•Ëá™Âä®Ê∑ªÂä†Âà∞Ê≠§ÂàóË°®
...
public:
	...
	BOOL WINAPI SetIntAttribute(int eSet, int nValue)
	{
		...
		case ATTR_LcdFilterWeight:
			if (!nValue)
				pSettings->m_bUseCustomLcdFilter = false;	//‰º†NULLËøáÊù•Â∞±ÊòØÂÖ≥Èó≠Ëá™ÂÆö‰πâËøáÊª§Âô®
			else
			{
				pSettings->m_bUseCustomLcdFilter = true;	//Âê¶ÂàôÊâìÂºÄËøáÊª§Âô®
				if (!IsBadReadPtr((void*)nValue, sizeof(pSettings->m_arrLcdFilterWeights)))	//Â¶ÇÊûúÊåáÈíàÊúâÊïà
					memcpy(pSettings->m_arrLcdFilterWeights, (void*)nValue, sizeof(pSettings->m_arrLcdFilterWeights));	//Â§çÂà∂Êï∞ÊçÆ
			}
			UpdateLcdFilter();	//Âà∑Êñ∞ËøáÊª§Âô®
			break;
		...
		case ATTR_ShadowBuffer:
			if (nValue && !IsBadReadPtr((void*)nValue, sizeof(pSettings->m_nShadow)))	//ÊåáÈíàÊúâÊïà
			{
				...
				if (token.GetCount()>=4)	//»Áπ˚÷∏∂®¡À«≥…´“ı”∞
					pSettings->m_nShadowDarkColor = pSettings->_httoi(token.GetArgument(3));	//∂¡»°“ı”∞
				else
					pSettings->m_nShadowDarkColor = 0;	//∑Ò‘ÚŒ™∫⁄…´
				if (token.GetCount()>=6)	//»Áπ˚÷∏∂®¡À…Ó…´“ı”∞
				{
					pSettings->m_nShadowLightColor = pSettings->_httoi(token.GetArgument(5));	//∂¡»°“ı”∞
					pSettings->m_nShadow[3] = pSettings->_StrToInt(token.GetArgument(4), pSettings->m_nShadow[2]); //∂¡»°…Ó∂»
				}
				else
				{
					//pSettings->m_nShadowLightColor = pSettings->m_nShadowLightColor;		//∑Ò‘Ú∫Õ«≥…´“ı”∞œ‡Õ¨
					pSettings->m_nShadow[3] = pSettings->m_nShadow[2];		//…Ó∂»“≤œ‡Õ¨
				}
				RefreshAlphaTable();
			}
			break;
	...
	{ 
		...
		if (*p) {
			...
			GetFontLocalName(p, buff); //ËΩ¨Êç¢Â≠ó‰ΩìÂêç
