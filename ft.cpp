/* 2006-10-23(by 555)
* http://hp.vector.co.jp/authors/VA028002/winfreetype.c (higambana(›—“∞”—ºo))
* §ÚÕË–¥§∑
*/
/* 2006-10-27(by 555)
* http://hp.vector.co.jp/authors/VA028002/freetype.html (higambana(›—“∞”—ºo))
* „ÇíÂèÇËÄÉ„Å´„Åó„Å¶„ÇÑ„ÇäÁõ¥„Åó
*/
/* 2006-10-29(by 555)
* 693Ê∞è(„Å®Âëº„Å∂„Åì„Å®„Å´„Åô„Çã)„ÅÆÁ≤æÂäõÁöÑ„Å™Ê¥ªÂãï„Å´„Çà„Å£„Å¶Âá∫Êù•‰∏ä„Åå„Å£„Åü„Ç¶„Éè„Ç¶„Éè„ÇΩ„Éº„Çπ„Å®
* ‰∏äË®ò„Çµ„Ç§„Éà„ÅÆÂ§âÊõ¥ÁÇπ„ÇíÂÖÉ„Å´„Åø„Åø„Å£„Å°„ÅÑ‰øÆÊ≠£„ÄÇ(„Éô„Éº„Çπgdi0164)
*/
/* (by 555)
* „Åï„Çâ„Å´Á∑öÂºï„Åç„ÇÇ„Ç¶„Éè„Ç¶„Éè„Å´„Åó„Å¶„ÇÇ„Çâ„Å£„Åügdi0168„ÇíÂÖÉ„Å´
* „Ç§„Çø„É™„ÉÉ„ÇØ„Å®„Éú„Éº„É´„Éâ„ÇíËøΩÂä†„ÄÇ
*/
/* (by sy567)
* Â§™Â≠ó„ÅÆ„Ç¢„É´„Ç¥„É™„Ç∫„É†„ÇíÂ§âÊõ¥„ÄÇ
* „Ç¨„É≥„ÉûË£úÊ≠£„ÇíÂÆüË£Ö„Åó„Å¶„Åø„Çã„ÄÇ
*/
#include "override.h"
#include "ft.h"
#include <windows.h>
...
FT_BitmapGlyphRec empty_glyph = {};//”≈ªØøÿ÷∆◊÷
...
{
	if (paramalpha == 1) {	//Ëé∑ÂèñÊñáÂ≠óÊ∑∑ÂêàË°®
		if (lcd) {
			tblR = tunetblR.data();
			tblG = tunetblG.data();
			tblB = tunetblB.data();
...
void CAlphaBlend::init()
{
	...
	for (i = 0; i < 256; ++i) {
		...
		tunetblBS[i] = Bound(0, alphatbl[Bound(tableB[i] * paramalpha / 100, 0, 255)], CAlphaBlend::BASE);	//ÊµÖËâ≤Ê∑∑ÂêàË°®
		...
		tunetblLBS[i] = Bound(0, alphatbl[Bound(tableB[i] * lightparamalpha / 100, 0, 255)], CAlphaBlend::BASE);	//Ê∑±Ëâ≤Ê∑∑ÂêàË°®
	}
}
...
// LCD(âtèª)ópÇÃÉAÉãÉtÉ@ÉuÉåÉìÉh(ÉTÉuÉsÉNÉZÉãÉåÉìÉ_ÉäÉìÉO)
static FORCEINLINE
...
// LCD(âtèª)ópï`âÊ(ÉTÉuÉsÉNÉZÉãÉåÉìÉ_ÉäÉìÉO)
// RGBèá(ÇÃÇÕÇ∏)
		...
		for (j = 0, dy = y; j < height; ++j, ++dy) {
			...
			for (i = left, dx = x; i < width; i += 3, ++dx) {
				...
				if (AAMode == 2 || AAMode == 4) {
					// Ç±ÇÍÇÕRGBÇÃèáÇ…ÉTÉuÉsÉNÉZÉãÇ™Ç†ÇÈÉfÉBÉXÉvÉåÉCóp
					// Ç±ÇÍÇÕRGBÇÃèáÇ…ÉTÉuÉsÉNÉZÉãÇ™Ç†ÇÈÉfÉBÉXÉvÉåÉCóp
...
// ÉOÉäÉtÉrÉbÉgÉ}ÉbÉvÇÃÉåÉìÉ_ÉäÉìÉO
	...
	int x, int y)
{
	if (FTGInfo.FTGlyph->bitmap.pixel_mode != FT_PIXEL_MODE_GRAY) {
		// Ç±ÇÃä÷êîé©ëÃÇÕFT_PIXEL_MODE_GRAYÇ…ÇÃÇ›ëŒâûÇµëºÇ…àœè˜Ç∑ÇÈ
...
// ècèëÇ´ópÇÃÉåÉìÉ_ÉäÉìÉO(ÉRÉsÉyéËî≤Ç´)
// 2äKí≤
	...
	CAlphaBlendColor& ab, int x, int y)
{
	...
	for (j = 0, dy = x; j < height; ++j, ++dy) {
		...
		for (i = 0, dx = y + width; i < width; ++i, --dx) {
			if ((p[i / 8] & (1 << (7 - (i % 8)))) != 0) {
				if (cache.GetPixel(dx, dy) != CLR_INVALID) { // dx dy ÉGÉâÅ[É`ÉFÉbÉN
...
CGGOGlyphLoader::init(FT_Library freetype_library)
{
	...
	//ëOÇÃï˚ñ@ÇæÇ∆ÅAarial.ttfÇ™ñ≥Ç¢Ç∆Ç‹Ç∏ÇªÇ§Ç»ÇÃÇ≈
	//ìKìñÇ…égÇ¶ÇÈÉAÉEÉgÉâÉCÉìÉtÉHÉìÉgÇíTÇ∑
...
BOOL FreeTypePrepare(FreeTypeDrawInfo& FTInfo)
{
	...
	FTInfo.params->lplf->lfWeight = FTInfo.params->otm->otmTextMetrics.tmWeight;	//Êõ¥Êñ∞Âà∞Ê†áÂáÜweight
	...
	case SETTING_FONTLOADER_FREETYPE:
	{
		...
		height = FTInfo.params->otm->otmTextMetrics.tmHeight - FTInfo.params->otm->otmTextMetrics.tmInternalLeading;	//Snowie!!ºÙµÙø’∞◊∏ﬂ∂»£¨bugfix°£
																														// 				if(lf.lfHeight > 0){
		...
		if (bVertical)
			swap(scaler.width, scaler.height);//»Áπ˚ « ˙œÚ◊÷ÃÂ£¨ΩªªªøÌ∏ﬂ
											  //!!Snowie
		scaler.pixel = 1;
		scaler.x_res = 0;
		scaler.y_res = 0;
	...
	case SETTING_FONTLOADER_FREETYPE:
		...
		/* ÉrÉbÉgÉ}ÉbÉvÇ‹Ç≈ÉLÉÉÉbÉVÉÖÇ∑ÇÈèÍçáÇÕFT_LOAD_RENDER | FT_LOAD_TARGET_*
		* Ç∆Ç∑ÇÈÅBÇΩÇæÇµìríÜÇ≈TARGETÇïœçXÇµÇΩèÍçáìôÇÕÉLÉÉÉbÉVÉÖÇ™é◊ñÇÇ∑ÇÈÅB
		* ÇªÇ§Ç¢Ç§éûÇÕFT_LOAD_DEFAULTÇ…ÇµÇƒFTC_ImageCache_Lookupå„Ç…
		* FT_Glyph_To_BitmapÇµÇΩÇŸÇ§Ç™ìsçáÇ™Ç¢Ç¢Ç∆évÇ§ÅB
		*/
		// BoldÇÕëæÇËãÔçáÇ∆Ç¢Ç§Ç‡ÇÃÇ™Ç†ÇÈÇÃÇ≈ñ{ìñÇÕÇ±ÇÍÇæÇØÇ≈ÇÕë´ÇËÇ»Ç¢ãCÇ™Ç∑ÇÈÅB
		/*if(IsFontBold(lf) && !(freetype_face->style_flags & FT_STYLE_FLAG_BOLD) ||
		lf.lfItalic && !(freetype_face->style_flags & FT_STYLE_FLAG_ITALIC)){
		// É{Å[ÉãÉhÅAÉCÉ^ÉäÉbÉNÇÕå„Ç≈ÉåÉìÉ_ÉäÉìÉOÇ∑ÇÈ
		// ëΩè≠ë¨ìxÇÕóÚâªÇ∑ÇÈÇæÇÎÇ§ÇØÇ«édï˚Ç»Ç¢ÅB
	...
	//Â¶ÇÊûúÂê´ÊúâÂÜÖÁΩÆhintingÂàôÂêØÁî®defaultÊ®°ÂºèÔºåÂê¶Âàô‰ΩøÁî®autohintÊ®°ÂºèÔºå‰ª•‰øùËØÅÊïàÊûú
	// ÉAÉìÉ`ÉGÉCÉäÉAÉX
	if (FTInfo.IsMono()) {
	...
	if (pSettings->HintSmallFont() /*&& font_type.flags & FT_LOAD_TARGET_LIGHT*/ && font_type.height != -1 && font_type.height<12)  //Õ®”√…Ë÷√≤ª π”√hinting£¨µ´ «¥Úø™¡À–°◊÷ÃÂhintingø™πÿ
	{
		/*
		if (!(freetype_face->face_flags & FT_FACE_FLAG_TRICKY))	//Â¶ÇÊûú‰∏çÊòØtrickyÂ≠ó‰Ωì
...
// ècÇ…Ç∑ÇÈÇ‚Ç¬ÇÕtrue(ASCIIÇ∆îºäpÉJÉiÇÕfalse)
inline bool IsVerticalChar(WCHAR wch) {
	if (wch < 0x80)
		return false;
	if (0xFF61 <= wch && wch <= 0xFF9F)
		return false;
	// ñ{ìñÇÕÇ‡Ç¡Ç∆ê^ñ ñ⁄Ç…Ç‚ÇÁÇ»Ç¢Ç∆Ç‹Ç∏Ç¢Ç™ÅB
	return true;
}
...
BOOL ForEachGetGlyphFT(FreeTypeDrawInfo& FTInfo, LPCTSTR lpString, int cbString, FT_Referenced_Glyph* GlyphArray, FT_DRAW_STATE* drState)
{
	...
	BOOL bAllowDefaultLink = pSettings->GetFontLinkInfo().IsAllowFontLink((BYTE)GetTextCharsetInfo(FTInfo.hdc, NULL, 0));	// «∑ÒŒ™∑˚∫≈
	BOOL nRet = true;
	BOOL bWindowsLink = pSettings->FontLink() == 2;
	//!!Snowie

	/*const*/ FT_Face freetype_face = FTInfo.freetype_face;	//ÂéªÊéâÂ∏∏ÈáèÂ±ûÊÄßÔºå‰∏ãÈù¢Ë¶ÅÊîπ‰ªñ
	...
	HFONT hOldFont = NULL;
	if (!bGlyphIndex && bWindowsLink)	// π”√Windows fontlink
	{
		lpfontlink = (WORD**)new LPVOID[FTInfo.face_id_list_num];
		for (int i = 0; i<LinkNum; i++)
		{
			lpfontlink[i] = new WORD[cbString];
			ZeroMemory(lpfontlink[i], sizeof(WORD)*cbString);	//≥ı ºªØŒ™Œﬁ¡¥Ω”
		}
		//
		hOldFont = (HFONT)GetCurrentObject(FTInfo.hdc, OBJ_FONT);	//Âä†ËΩΩÁ¨¨‰∏Ä‰∏™Â≠ó‰Ωì
	}
	//fontlink
	...
	if (!bAllowDefaultLink && FTInfo.face_id_list_num > 1)
		FTInfo.face_id_list_num--;	//Â¶ÇÊûúÊòØsymbolÈ°µÈÇ£Â∞±‰∏çÈìæÊé•Âà∞ÂÆã‰Ωì
	...
	for (int i = 0; lpString < lpEnd; ++lpString, ++gi, ++GlyphArray, ++drState, ++AAList, /*ggdi32++,*/ i++) {
		...
		if (!*glyph_bitmap) {	// case: no cache found
			...
			if (bGlyphIndex) {	// glyph index doesn't require any font linking
				...
				glyph_index = wch;
				*AAList = AAMode;
				GetCharWidthI(FTInfo.hdc, wch, 1, (LPWORD)&wch, &gdi32x);	//indexÁöÑÊñáÂ≠óÂøÖÈ°ªËÆ°ÁÆóÂÆΩÂ∫¶
				if (FTInfo.font_type.height <= pSettings->BitmapHeight() && pfi->EmbeddedBmpExist(FTInfo.font_type.height))
				{
					f_glyph = false;	// π”√µ„’Û£¨≤ªªÊÕº
					*drState = FT_DRAW_EMBEDDED_BITMAP;	//…Ë÷√Œ™µ„’ÛªÊÕº∑Ω Ω
				}
			}
			else
				if (wch && !CID.myiswcntrl(lpString[0])) {	// need to draw a non-control character				
					for (int j = 0; j < FTInfo.face_id_list_num; ++j) {
						...
						if (bWindowsLink)	//‰ΩøÁî®WindowsÂáΩÊï∞ËøõË°åfontlink
						{
							if (!lpfontlink[j][i])	//ªπ√ª≥ı ºªØ∏√◊÷ÃÂµƒfontlink
							{
								SelectFont(FTInfo.hdc, FTInfo.ggo_font_list[j]);	//Âä†ËΩΩggoÂ≠ó‰Ωì
								GetGlyphIndices(FTInfo.hdc, lpString, cbString - i, &lpfontlink[j][i], GGI_MARK_NONEXISTING_GLYPHS);	//Ω¯––fontlink
						...
						if (glyph_index) {
							GetCharWidth32W(FTInfo.hdc, wch, wch, &gdi32x);	//”––ßŒƒ◊÷£¨º∆À„øÌ∂»
							f_glyph = true;
							FTInfo.font_type.face_id = FTInfo.face_id_list[j];
							freetype_face = FTInfo.GetFace(j);	//ÂêåÊó∂Êõ¥Êñ∞ÂØπÂ∫îfaceidÁöÑÂÆûÈôÖface
																//Êé•‰∏ãÊù•Êõ¥Êñ∞ÂØπÂ∫îÁöÑfontsetting
							...
							if (FTInfo.font_type.face_id == FTInfo.face_id_simsun && j>0)
							{
								...
								{
								case 11: {FTInfo.font_type.height = 12; FTInfo.font_type.width++; break; }	//ÂØπÂÆã‰ΩìËøõË°åÁâπÊÆäÂ§ÑÁêÜ
							...
							if (pfi)
							{
								...
								if (pSettings->HintSmallFont() && FTInfo.font_type.flags & FT_LOAD_TARGET_LIGHT && FTInfo.font_type.height != -1 && FTInfo.font_type.height<12)  //Õ®”√…Ë÷√≤ª π”√hinting£¨µ´ «¥Úø™¡À–°◊÷ÃÂhintingø™πÿ
			...
			if (!f_glyph || !freetype_face) {	//can't find suitable fontface, glyphindex case is already calculated.
				...
				if (*drState == FT_DRAW_NORMAL || bGlyphIndex)
					*drState = FT_DRAW_NOTFOUND;	//’“≤ªµΩŒƒ◊÷
				if ((!FTInfo.lpDx || lpString == lpEnd - 1) && !bGlyphIndex)	//Œﬁ–ßŒƒ◊÷£¨∂¯«“√ª”– ¬œ»≈≈∞ÊªÚ’ﬂ «≈≈∞Êµƒ◊Ó∫Û“ª∏ˆ◊÷∑˚¡À
				{
					GetCharWidth32W(FTInfo.hdc, wch, wch, &gdi32x);
				}
				int cx = gdi32x;
...

				{
					...
					// 					else
					// 						if (isc == CNTRL_ZERO_WIDTH)	//‘§º∆À„µƒŒﬁøÌ∂»øÿ÷∆◊÷
					// 							cx = 0;
					int dyHeight = clpdx.gety(0);
					int dxWidth = clpdx.get(cx);

					if (isc == CNTRL_COMPLEX_TEXT)	//øÿ÷∆◊÷
					{
						cx = dxWidth;	//Êúç‰ªéwindowsÁöÑÂÆΩÂ∫¶Ë∞ÉÂ∫¶
										//if (!dxWidth)
					...
					if (lpString < lpEnd - 1) {
						...
						//FTInfo.px = FTInfo.x+Max(clpdx.get(cx), abc.abcA+(int)abc.abcB+abc.abcC);	//Êó†ÊïàÊñáÂ≠óÁöÑÊÉÖÂÜµ‰∏ãÔºåÁªòÂõæÂÆΩÂ∫¶=Èº†Ê†á‰ΩçÁΩÆ
			...
			if (bVertical && IsVerticalChar(wch)) {
				...
				if (bVertical)
					swap(FTInfo.font_type.height, FTInfo.font_type.width);	//‰∫§Êç¢Êó†Ê≥ïÊóãËΩ¨ÁöÑÊñáÂ≠óÂÆΩÈ´ò
				FTInfo.font_type.flags &= ~FT_LOAD_VERTICAL_LAYOUT;
				if (bLcdMode) {
...

			{
				...
				bIsIndivBold = freetype_face->style_flags & FT_STYLE_FLAG_BOLD;	//ÊòØÁã¨Á´ãÁ≤ó‰Ωì
				bIsBold = (IsFontBold(lf) && !bIsIndivBold);	//ÊòØ‰ªøÁ≤ó‰Ωì
...

				{
					...
					FT_Glyph_Copy(temp_glyph, &(glyph->ft_glyph));	//◊™ªªŒ™ref_glyph
				}
		...
		{
			int dy = clpdx.gety(0);	//ªÒµ√∏ﬂ∂»
			int dx = clpdx.get(bWidthGDI32 ? gdi32x : cx);	//ªÒµ√øÌ∂»
			...
			if (gdi32x == 0) {	// zero width text (most likely a diacritic)
				...
				if (FTInfo.x + left < FTInfo.xBase)
					FTInfo.xBase = FTInfo.x + left;	//Â¶ÇÊûúÊúâÂ≠óÁ¨¶ÊòØË¥üÊï∞Ëµ∑Âßã‰ΩçÁΩÆÁöÑÔºàÂêàÊàêÁ¨¶Âè∑ÔºâÔºå Ë∞ÉÊï¥ÊñáÂ≠óÁöÑËµ∑Âßã‰ΩçÁΩÆ
			}
			...
			if (lpString < lpEnd - 1) {
				...
				bx += left;
				FTInfo.px = FTInfo.x + Max(Max(dx, bx), cx);	//ÊúâÊñáÂ≠óÁöÑÊÉÖÂÜµ‰∏ã,ÁªòÂõæÂÆΩÂ∫¶=ftËÆ°ÁÆóÁöÑÂÆΩÂ∫¶ÔºåÈº†Ê†á‰ΩçÁΩÆ=winÂÆΩÂ∫¶
	...
	cont:
		*Dx = FTInfo.x;		//DxµƒŒª÷√ «œ¬“ª∏ˆ◊÷∑˚ø™ ºµƒª˘◊ºŒª÷√£¨≤¢≤ª «œ¬“ª∏ˆ◊÷∑˚ø™ ºª≠µƒŒª÷√
		*Dy = FTInfo.y;		//DyÁöÑ‰ΩçÁΩÆÊòØ‰∏ã‰∏Ä‰∏™Â≠óÁ¨¶ÁöÑyÂùêÊ†á
		++Dx;
		++Dy;
	}
gdiexit:
...
BOOL ForEachGetGlyphGGO(FreeTypeDrawInfo& FTInfo, LPCTSTR lpString, int cbString, FT_Referenced_Glyph* GlyphArray, FT_DRAW_STATE* drState)
{
	...
	/*
	if (!FTInfo.lpDx)	//√ª”–‘§œ»º∆À„≈≈∞Ê£¨–Ë“™ªÒµ√√ø∏ˆŒƒ◊÷µƒøÌ∂»–≈œ¢
	{
	if (bGlyphIndex)
	{
	...
	{
	//‘§œ»º∆À„∫√¡À≈≈∞Ê£¨÷ª–Ë“™ªÒµ√◊Ó∫Û“ª∏ˆ◊÷µƒ–≈œ¢æÕø…“‘¡À
	if (bGlyphIndex)
	{
	...
	if (!bGlyphIndex)  	//‰ªÖÂØπwin32ÊÉÖÂÜµËøõË°å‰ºòÂåñÔºåftÊÉÖÂÜµÂè¶ËÆÆ
	...
	for (int i = 0; lpString < lpEnd; ++lpString, gi++, GlyphArray++, drState++, ++AAList,/*ggdi32++,*/ i++) {
		...
		if (!*glyph_bitmap) {
			...
			if (!f_glyph) {	//glyphindexÁöÑÊñáÂ≠ó‰∏äÈù¢Â∑≤ÁªèËÆ°ÁÆóËøá‰∫Ü
#ifdef _DEBUG
				GdiSetBatchLimit(0);
#endif
				if (*drState == FT_DRAW_NORMAL || bGlyphIndex)
...
BOOL CALLBACK TextOutCallback(FreeTypeGlyphInfo& FTGInfo)
{
	...
	if (!FTGInfo.FTGlyph->bitmap.buffer) {
		//if (FTInfo->params->alpha == 1) {
		// 		if (!(FTInfo->GetETO() & ETO_GLYPH_INDEX) && wch==32)	//Á©∫Ê†º
		...
		if (bVertical && IsVerticalChar(FTGInfo.wch) &&
			...
			if (FTInfo->params->alpha>1)
			{
				FreeTypeDrawBitmapV(FTGInfo, *FTGInfo.shadow, FTInfo->x + FTInfo->sx,
					FTInfo->yTop + FTInfo->params->otm->otmTextMetrics.tmHeight - (glyph_bitmap->left + glyph_bitmap->bitmap.width) - 1 + FTInfo->sy);//ª≠“ı”∞
				FTInfo->params->alpha = 1;
			}
			if (!FreeTypeDrawBitmapV(FTGInfo, *FTGInfo.solid, FTInfo->x,
				FTInfo->yTop + FTInfo->params->otm->otmTextMetrics.tmHeight - (glyph_bitmap->left + glyph_bitmap->bitmap.width) - 1))	//ª≠Œƒ◊÷	
			{
			...
			if (FTInfo->params->alpha>1)
			{
				FreeTypeDrawBitmap(FTGInfo, *FTGInfo.shadow,
					...
					FTInfo->yTop + FTInfo->yBase - glyph_bitmap->top + FTInfo->sy);	//ª≠“ı”∞
				FTInfo->params->alpha = 1;
			}
			if (!FreeTypeDrawBitmap(FTGInfo, *FTGInfo.solid,
				FTInfo->x + glyph_bitmap->left,
				FTInfo->yTop + FTInfo->yBase - glyph_bitmap->top))	//ª≠Œƒ◊÷
			{
				// fallback to GDI when fail to draw with FT
				ORIG_ExtTextOutW(FTInfo->hdc, FTInfo->x, FTInfo->yTop, FTInfo->GetETO(), NULL, &FTGInfo.wch, 1, NULL);
			}
...
{
	//return (GetRValue(Color)*0.299 + GetGValue(Color)*0.587 + GetBValue(Color)*0.114);	//‘≠ ºÀ„∑®
	//===============================================================
	//≤…”√Photoshop sRGBµƒRGB->LabÀ„∑®Ω¯––ªªÀ„£¨LŒ™…´≤  ”æı¡¡∂»
	//ÊÑüË∞¢ Ë•øÂÆâÁêÜÂ∑•Â§ßÂ≠¶ Ë¥æÂ©â‰∏Ω ÁöÑÂàÜÊûê
	//===============================================================
	static double s_multipler = 116 / pow(100, (double)1.0 / 3.0);	//È¢ÑËÆ°ÁÆóÂ∏∏Êï∞,Âº∫Âà∂‰ΩøÁî®doubleÁâàÊú¨
	double* RGBTable = s_AlphaBlendTable.GetRGBTable();	//Ëé∑ÂæóÊòæÁ§∫Âô®ËΩ¨Êç¢Ë°®
...
BOOL FreeTypeTextOut(
	const HDC hdc,     // ÉfÉoÉCÉXÉRÉìÉeÉLÉXÉgÇÃÉnÉìÉhÉã
	CBitmapCache& cache,
	LPCWSTR lpString,  // ï∂éöóÒ
	int cbString,      // ï∂éöêî
...
)
{
	...
	if (FTInfo.params->alpha != 1)
	{
		...
		else
		{
			...
			if (diff<10)
				...
				FTInfo.params->alphatuner = max(1, 100 / diff);	//∏˘æ›…´≤Óµ˜’˚“ı”∞≈®∂»
		}
	}
	...
	//ËÆ°ÁÆó‰∏ãÂàíÁ∫øÊàñÂà†Èô§Á∫øÁöÑ‰ø°ÊÅØ
	...
	// 		if(lf.lfUnderline){
	// 			int yPos = 0; //â∫ê¸ÇÃà íu
	// 			int height = 0;
	// 			int thickness = 0; // ìKìñÇ»ëæÇ≥
	// 			switch (pSettings->FontLoader()) {
	// 			case SETTING_FONTLOADER_FREETYPE:
...
BOOL FreeTypeGetGlyph(	//ªÒµ√À˘”–Õº–Œ∫Õ–Ë“™µƒøÌ∂»
	FreeTypeDrawInfo& FTInfo,
	LPCWSTR lpString,
	int cbString,
	int& width,
...
//
// ÉOÉäÉtÇIVSÇ≈éwíËÇ≥ÇÍÇΩéöå`ÇÉTÉ|Å[ÉgÇ∑ÇÈÇ©Ç«Ç§Ç©í≤Ç◊ÅA
// ÉTÉ|Å[ÉgÇµÇƒÇ¢ÇÈèÍçáÇÕÉOÉäÉtÇíuä∑Ç∑ÇÈÅB
// ÉTÉ|Å[ÉgÇµÇƒÇ¢Ç»ÇØÇÍÇŒâΩÇ‡ÇµÇ»Ç¢ÅB
//
/*
...
if (glyph_index) {
pwOutGlyphs[*pcGlyphs - 1] = glyph_index; // íuä∑Çé¿çs
// ASCIIãÛîíÇÃÉOÉäÉtÇéÊìæ
...
// ÉtÉHÉìÉgÇÕéwíËÇ≥ÇÍÇΩéöå`ÇéùÇΩÇ»Ç¢ÅBIVSÇÃÉOÉäÉtÇéÊìæ
glyph_index = FTC_CMapCache_Lookup(
cmap_cache,
FTInfo.font_type.face_id,
FTInfo.cmap_index,
vsindex + 0xE0100);
// IVSÇÉTÉ|Å[ÉgÇµÇƒÇ¢Ç»Ç¢ÉtÉHÉìÉgÇÕIVSÇÃÉOÉäÉtÇéùÇ¡ÇƒÇ¢ÇÈâ¬î\ê´Ç‡ÇŸÇ∆ÇÒÇ«Ç»Ç¢ÅB
// missing glyphÇï‘Ç∑Ç∆ÉtÉHÅ[ÉãÉoÉbÉNÇ≥ÇÍÇƒÇµÇ‹Ç§ÇΩÇﬂämé¿Ç…éùÇ¡ÇƒÇ¢ÇªÇ§Ç»ÉOÉäÉtÇèEÇ§
if (!glyph_index)
glyph_index = FTC_CMapCache_Lookup(
cmap_cache,
FTInfo.font_type.face_id,
...
//â°ï˚å¸ÇæÇØëæÇÁÇπÇÈFT_Outline_Embolden
FT_Error Old_FT_Outline_Embolden(FT_Outline*  outline, FT_Pos strength)
{
	FT_Vector*	points;
	...
	for (c = 0; c < outline->n_contours; c++)
	{
		...
		for (n = first; n <= last; n++)
		{
			...
			//Å´Ç±ÇÍÇÉRÉÅÉìÉgÉAÉEÉgÇµÇΩÇæÇØ
