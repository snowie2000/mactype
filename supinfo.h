...
// 		WriteFile(logfile, "\xff\xfe", 2, &aa, NULL);
	LPTSTR vlpApp = (LPTSTR)lpApp;	//ÂèòÊàêÂèØ‰ª•Êìç‰ΩúÁöÑÂèÇÊï∞
	if (lpApp)
	{
		do 
		{
...
// 			WriteFile(logfile, _T("\n"), 2, &aa, NULL);
			vlpApp = _tcsstr(vlpApp+1, _T(" "));	//ªÒµ√µ⁄“ª∏ˆø’∏ÒÀ˘‘⁄µƒŒª÷√
			ret.assign(lpApp);
			if (vlpApp)
...
// 			WriteFile(logfile, _T("\n"), 2, &aa, NULL);
			...
			if (fa!=INVALID_FILE_ATTRIBUTES && fa!=FILE_ATTRIBUTE_DIRECTORY)	//Êñá‰ª∂ÊòØÂê¶Â≠òÂú®
			{		
				int p = ret.find_last_of(_T("\\"));
				if (p!=-1)
					ret.erase(0, p+1);	//»Áπ˚”–¬∑æ∂æÕ…æµÙ¬∑æ∂
// 				WriteFile(logfile, ret.c_str(), ret.length()*2, &aa, NULL);
// 				WriteFile(logfile, _T("\n"), 2, &aa, NULL);
// 				WriteFile(logfile, _T("==========\n"), 24, &aa, NULL);
// 				CloseHandle(logfile);
...
// 		WriteFile(logfile, lpCmd, _tcslen(lpCmd)*2, &aa, NULL);
		...
		if ((*lpCmd)==_T('\"'))
		{
			ret.erase(0,1);	//Âà†Èô§Á¨¨‰∏Ä‰∏™ÂºïÂè∑
			p=ret.find_first_of(_T("\""));	//Êü•Êâæ‰∏ã‰∏Ä‰∏™ÂºïÂè∑
		}
		else
			p=ret.find_first_of(_T(" "));
		if (p>0)
			ret.resize(p);	//Ëé∑ÂæóCmdÈáåÈù¢ÁöÑÊñá‰ª∂Âêç
// 		WriteFile(logfile, ret.c_str(), ret.length()*2, &aa, NULL);
// 		WriteFile(logfile, _T("\n"), 2, &aa, NULL);
